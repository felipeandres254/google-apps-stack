...
function UnitTest( unit_name ) {
	...
	var Test = function Test(name, test) {
		...
		this.run = function() {
			try {
				...
				this.result = {"outcome":"ERROR", "message":e.message, "error":e};
			}
		};
		...
		this.parseHTML = function() {
			...
			html += "<span><b>" + this.name + "</b> " + this.result.outcome.message + "</span>";
	...
	this.exportHTML = function( show ) {
		...
		var css = [
			...
			"ul.results { list-style:none; padding-left:25px; } li.result:before { content:\"\"; width:25px; height:25px; margin-left:-25px; float:left; display:block; } li.result span { line-height:25px; } li.result pre { margin-top:0px; }",
		...
		var html = css + "<ul class=\"results\">" + this.tests.map(function(test) { return test.parseHTML(); }).join("") + "</ul>";
		if( show )
			SpreadsheetApp.getUi().showModalDialog(HtmlService.createHtmlOutput(html), "Test results - " + this.name);
		return html;
	}
